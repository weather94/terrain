...
export default class Deploy extends Command {
  ...
  static flags = {
    ...
    signer: flags.string({ required: true, default: "admin" }),
  ...
  async run() {
    ...
    if (args.contract === "all") {
      ...
      contracts = args.contract.split(",");
    }
    console.log("contracts", contracts);

    for (let i = 0; i < contracts.length; i++) {
      const contract = contracts[i];
      const configPath = fs.existsSync(`./config/${contract}.json`)
        ? `./config/${contract}.json`
        : flags["config-path"];

      const connections = loadConnections(configPath);
      const config = loadConfig(configPath);
      const conf = config(flags.network, contract);
